...
int str_compar(char *str, char *example) {
    for (int i = strlen(example); i > 0; i--) if (str[i-1] != example[i-1]) return 0;
    return 1;
}
...
int files_check(char *path, char files[][30], char *missing_files, int size) {
    ...
    for (int i = 0; i < size; i++) {
        ...
        if (access(full_path, F_OK) == 0) continue;
            ...
            if (str_compar(files[i], "files\\ide\\config.json") == 1) {
                create_config(path);
                continue;
            }

            else if (str_compar(files[i], "files\\ide\\libraries.json") == 1) {
                FILE *file = fopen(full_path, "w");
                fprintf(file, "[\n\t\n]");
                continue;
            }
...
int main(void) {
    ...
    enum { icons_size = 9, files_size = 6 };
    ...
    char files[files_size][30] = {"gui.pyw", "rn_gui.py", "setup.bat", "vcc.pyw", "files\\ide\\config.json", "files\\ide\\libraries.json"};
    char directories[6][16] = {"files\\ide", "files\\libraries", "icons", "projects", "outputs\\bin", "outputs\\hex"};
